...
class InverseLinear(torch.nn.Module):
    ...
    def state_dict(self):
        return {"weight": self.fc.weight, "inverse_bias": self.inverse_bias}
...
class NeuronActAdd(torch.nn.Module):
    ...
    def set_actadd(self, offset: Tensor):
        ...
        self.param = torch.nn.Parameter(offset.to(device=self.device, dtype=self.dtype))
        self.reset()
...
def mlp_svd_two_layer_raw(
    ...
    inv_out = (
        InverseLinear(
            original_weights=W_out,
            original_biases=b_out,
            n_heads=n_heads,
        )
        .to(dtype=dtype)
        .to(device)
