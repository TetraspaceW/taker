...
from .nn import (
    ...
    NeuronActAdd,
    mlp_delete_rows_raw,
    mlp_svd_two_layer_raw,
    mlp_delete_columns_raw,
)
...
class Model:
    ...
    def import_models(self, tokenizer=None, predictor=None, processor=None):
        ...
        if self.cfg.model_modality == "vision":
            ...
            self.tokenizer = None if tokenizer is None else tokenizer
            self.processor = (
            ...
            self.tokenizer = (
                AutoTokenizer.from_pretrained(self.tokenizer_repo, legacy=False)
                if tokenizer is None
                else tokenizer
            )
            self.processor = None if processor is None else processor
            self.predictor = (
            ...
            raise NotImplementedError(
                f"Model modality {self.cfg.model_modality} not implemented."
            )
    ...
    def init_model(
        ...
        **kwargs,
        ...
        self.activations = {"attn": {}, "attn_pre_out": {}, "ff": {}, "mlp_pre_out": {}}
        self.do_activations = {
            "attn": True,
            "attn_pre_out": True,
            "ff": True,
    ...
    def remove_hooks_from(self, submodel):
        ...
        for module in submodel.modules():
            ...
            hooks = list(module._forward_pre_hooks.keys()) + list(
                module._forward_hooks.keys()
            )
    ...
    def register_activations(self):
        ...
        for layer_index, layer in enumerate(self.layers):
            ...
            if self.mlp_pre_out_mode == "hook":
                ...
                _handle = fc2.register_forward_pre_hook(
                    self.build_input_hook(component_name, name)
                )
                self.save_hook_handle(
                    _handle, "input-read", "mlp.out_proj", layer_index
                )
            ...
            if self.attn_pre_out_mode == "hook":
                ...
                self.do_activations[component_name] = True
                _handle = attn_o.register_forward_pre_hook(
                    self.build_input_hook(component_name, name)
                )
                self.save_hook_handle(
                    _handle, "input-read", "attn.out_proj", layer_index
                )

        print(f" - Registered {layer_index+1} Attention Layers")

    def register_input_mask(
        self, module: torch.nn.Module, component: str, layer_index: int
    ):
    ...
    def register_post_biases(self):
        ...
        do_attn_vo_biases = (
            "attn.v_proj" in self.layers[0]
            and self.layers[0]["attn.v_proj"] is not None
        )
        ...
        if do_attn_vo_biases:
            for layer_index, layer in enumerate(self.layers):
                ...
                self.save_hook_handle(
                    _handle, "output-bias", "attn.out_proj", layer_index
                )
        ...
        if "mlp.out_proj" in self.layers[0]:
            for layer_index, layer in enumerate(self.layers):
                ...
                self.save_hook_handle(
                    _handle, "output-bias", "mlp.out_proj", layer_index
                )

            self.post_biases["mlp_out"] = NeuronFunctionList(
                self.post_biases["mlp_out"]
            )
    ...
    def run_masking(self, activations: Tensor, component: str):
        ...
        for layer_index in range(self.cfg.n_layers):
            ...
            masked_activations.append(mask(act.view(temp_shape)).view(orig_shape))
    ...
    def get_image_activations(self, pixel_values: Optional[Tensor] = None):
        """_summary_
        Gives the output of each major component of the transformer before being
        added to the residual_stream. i.e: ( input, attention_out, ff_out, output )
    ...
    def get_residual_stream(
        ...
        **kwargs,
        ...
        if text_activations is None:
            text_activations = self.get_text_activations(
                text, input_ids, inputs_embeds, limit, **kwargs
            )
        ...
        **kwargs,
        ...
        if residual_stream is None:
            residual_stream = self.get_residual_stream(
                text, input_ids, inputs_embeds, text_activations, limit, **kwargs
            )
        ...
        if self.mlp_pre_out_mode == "hook":
            ...
            ff_mids = self.calculate_ff_keys(
                ff_inputs.to(self.device), use_activation_function
            )
    ...
    def get_attn_pre_out_activations(
        self,
        text: Optional[str] = None,
        input_ids: Optional[Tensor] = None,
        inputs_embeds: Optional[Tensor] = None,
    ...
    def get_attn_value_activations(
        self,
        text: Optional[str] = None,
        input_ids: Optional[Tensor] = None,
        inputs_embeds: Optional[Tensor] = None,
    ...
    def calculate_attn_out_layer(
        self, attn_in: Tensor, layer: int, attention_mask: Tensor
    ):
        u = self.layers[layer]
        x = u["ln1"](attn_in)
    ...
    def calculate_attn_pre_out_layer(
        self, attn_out: Tensor, layer: int, reshape: bool, transpose: bool
    ):
    ...
    def calculate_attn_pre_out(
        self, attn_out: Tensor, reshape: bool = True, transpose: bool = False
    ):
    ...
    def calculate_attn_value_layer(
        self,
    ...
    def delete_attn_pre_out(
        ...
        if use_means:
            ...
            assert (
                torch.tensor(mean_activation.size()).prod()
                == torch.tensor(remove_indices.size()).prod()
            )

        for layer_index in range(self.cfg.n_layers):
            layer_mean_activation = mean_activation[layer_index] if use_means else None
            self.delete_attn_pre_out_layer(
                layer_index, remove_indices[layer_index], layer_mean_activation
            )
    ...
    def calculate_ff_keys(self, ff_in: Tensor, use_activation_function: bool = True):
        out = []
        for layer_index, ff_in_layer in enumerate(ff_in):
            out.append(
                self.calculate_ff_keys_layer(
                    ff_in_layer,
                    layer_index,
    ...
    def evaluate_ce_loss(
        ...
        """
        predicted_log_probs = self.evaluate_ce_losses(
            text, input_ids, expected_ids, logits
        )
    ...
    def roberta_masked_ids(
        ...
        input_ids[0, indices_randomized] = torch.randint(
            4, self.cfg.d_vocab - 1, (n_randomized,)
        ).to(device)
