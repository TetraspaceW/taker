...
most_common_pile_codeless_tokens = llama_most_common_tokens["only_text"]["skip50"][
    "tokens_str"
]
...
class DatasetFilters:
    ...
    def filter_fruit_and_vegetables(_dataset):
        ...
        fruit_and_vegetables_dataset = _dataset.filter(
            filter_fruit_and_vegetables_example
        )
    ...
    def filter_household_electrical_devices(_dataset):
        ...
        household_electrical_devices_dataset = _dataset.filter(
            filter_household_electrical_devices_example
        )
    ...
    def filter_household_furniture(_dataset):
        ...
        household_furniture_dataset = _dataset.filter(
            filter_household_furniture_example
        )
    ...
    def filter_pile_PubMed_Abstracts(_dataset):
        ...
        pile_PubMed_Abstracts_dataset = _dataset.filter(
            filter_pile_PubMed_Abstracts_example
        )
    ...
    def filter_pile_PubMed_Central(_dataset):
        ...
        pile_PubMed_Central_dataset = _dataset.filter(
            filter_pile_PubMed_Central_example
        )
    ...
    def filter_pile_USPTO_Backgrounds(_dataset):
        ...
        pile_USPTO_Backgrounds_dataset = _dataset.filter(
            filter_pile_USPTO_Backgrounds_example
        )
...
def infer_dataset_config(dataset_name: str, dataset_subset: str = None):
    eval_configs = [
        ...
        EvalConfig(
            ...
            dataset_repo="monology/pile-uncopyrighted",
            skip_token_strings=most_common_pile_codeless_tokens,
            dataset_filter=lambda __dataset: DatasetFilters.filter_pile_general(
                __dataset, "FreeLaw"
            ),
        ),
        EvalConfig(
...
def prepare_dataset(eval_config: EvalConfig):
    ...
    if eval_config.num_texts_to_skip >= 1:
        print(
            f"skipping {eval_config.num_texts_to_skip} texts in {eval_config.dataset_name}"
        )

        # Skip only works for DatasetIterable. Kinda annoying ngl
        if hasattr(_dataset, "skip"):
            _dataset = _dataset.skip(
                eval_config.num_texts_to_skip
            )  # Conservative skip limit
        else:
    ...
    if split == "train" and not eval_config.is_train_mode:
        ...
        print(
            "Warning: 'pile_deduped' has no 'test' split.",
