...
def load_tensors_for_repo(repo, model_size="hf", timestamp="recent"):
    directory = (
        "/home/tetraspace/Documents/projects/taker/examples/neuron-mapping/saved_tensors/"
        + model_size
        + "/"
    )
    filename = repo + "-cifar20-split-" + model_size + "-0.01-" + timestamp + ".pt"
...
def find_accuracy(pruning_dataset, target_dataset, ff_frac):
    ...
    model_size = c.model_size.split("-")[-1]

    print(f"Loading model from {model_size}")
...
def find_correct_ff_frac(
    dataset: str,
    target_accuracy: float,
    accuracy_precision: float,
    ff_frac_precision=1e-6,
    lower=0,
    upper=1,
):
    print(
    ...
    while upper >= lower + ff_frac_precision:
        ...
        if (
            ...
            lower = (lower + upper) / 2
    print(
        "reached limit of ff_frac_precision without reaching target accuracy, likely due to evaluation not being deterministic"
    )
    return lower
...
def compareEvaluations(datasets):
    ...
    for dataset1 in datasets:
        ...
        for dataset2 in datasets:
            ...
            final_data[dataset1][dataset2]["accuracy_difference"] = (
                unpruned_accuracy - pruned_accuracy
            )
            final_data[dataset1][dataset2]["accuracy_ratio"] = (
                pruned_accuracy / unpruned_accuracy
            )
        dataset_end = datetime.now()
        print(
            "time to get data for dataset: ",
            dataset1,
            "is ",
